	...
	for(int i=2; i<=1000; i++)
	{
		...
		if(pm)
		{
			if(i<100)
			{
				...
				prime[0][f1]=i;							//inserting prime no between 0-100 in first row
				f1++;
			}
			else if(i<200)
			{
				prime[1][f2]=i;							//inserting in second row
				f2++;
			}
			else if(i<300)
			{
				prime[2][f3]=i;								//inserting in third row
				f3++;
			}
			else if(i<400)
			{
		...
		//program to print calendar using stack	
			public static void calenderStack(int month, int year) 
			{
				String s[] = { "January", "Februry", "March", "April", "May", "June", "July", "August", "September", "October",
						"November", "December" };
				StackForCalendar<String> stack = new StackForCalendar<String>();
